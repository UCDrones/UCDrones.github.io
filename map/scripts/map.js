...
require([
    ...
    var uasFacilities = new FeatureLayer({
        ...
        popupTemplate: {
            title: "UAS Facility Map Grid",
            content: "<b>Airport:</b> {APT1_NAME}<br><b>LAANC:</b> {APT1_LAANC}<br><b>Ceiling:</b> {CEILING}",
        },
    });
    ...
    const view = new MapView({
        ...
        container: "Map",
        aria: {
            label: "Drone Map of California",
            description: "Interactive map showing airspace, public lands, and local regulations in California. Use the layer list and search controls to explore."
        },
...
// Add the locate widget to the top left corner of the view
    ...
    searchWidget.on("select-result", async (event) => {
        ...
        const location = geom.type === "extent" ? geom.center : geom;

        const features = [];

        // ✅ Iterate ALL layers (including GroupLayer children)
        const featureLayers = map.allLayers.filter(
            layer => layer.type === "feature" && layer.visible
        );

        for (const layer of featureLayers) {
            try {
                // Ensure layer + layerView are ready
                await layer.load();
                ...
                console.warn(
                    "Query failed for layer:",
                    layer.title || layer.id || layer.url,
                    err
                );
            }
        }
        ...
        if (!features.length) {
            view.openPopup({
                title: "No intersecting features",
                content: event.result.name,
                location
            });
            return;
        }

        // ✅ Open ONE popup with everything
        view.openPopup({
            features,
            location
        });
